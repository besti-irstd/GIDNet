...
def parse_args():
    ...
    # [新增] 控制是否使用 DHPF 模块。1 表示使用，0 表示禁用,在GIDNet中并没有使用频域模块。
...
class Trainer(object):
    def __init__(self, args):
        ...
        if args.mode=='test':
            ...
            weight = torch.load(args.weight_path,weights_only=False)
            if args.weight_path[-1] == 'r':
                weight = weight['state_dict']
            else:
                weight = weight
            self.model.load_state_dict(weight,False)
            '''
                # iou_67.87_weight
    ...
    def train(self, epoch):
        ...
        # tag 在循环内根据 use-dhpf 和 epoch 动态判定
        for i, (data, mask) in enumerate(tbar):
  
            data = data.to(self.device)
            labels = mask.to(self.device)

            # 判断是否启用 warm 阶段逻辑 (即 DHPF)
            # 只有当 use_dhpf 为 1 且 当前 epoch 大于 warm_epoch 时，tag 才为 True
            # warm阶段采用的Loss和不是warm阶段采用的Loss是不一样的，一定要注意区分
            if self.args.use_dhpf and epoch > self.warm_epoch:
                tag = True
            else:
                tag = False
